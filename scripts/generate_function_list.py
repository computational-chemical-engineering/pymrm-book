...
def render_module_page(
    ...
    for item in module_public_items:
        symbol_link = f"../symbols/{slugify_symbol(item['full_name'])}.md"
    ...
    for item in module_public_items:
        ...
        lines.extend(
            [
                ...
                "",
                f"[Open dedicated reference page](../symbols/{slugify_symbol(item['full_name'])}.md)",
                "",
            ]
        )
...
def render_symbol_page(item: dict, items: dict[str, dict], commit_sha: str) -> list[str]:
    ...
    lines = [
        f"# `{item['full_name']}`",
        "",
        f"[Back to module page](../modules/{slugify_symbol(module_name)}.md) · "
        "[Back to alphabetical overview](../alphabetical_overview.md)",
        "",
    ]
...
def main() -> None:
    ...
    for module_name in module_names:
        ...
        api_lines.extend(
            [
                f"## [`{module_name}`](modules/{slugify_symbol(module_name)}.md)",
                "",
    ...
    for item in public_items:
        summary, _ = split_summary(item.get("doc", ""))
        alpha_lines.append(
            f"| [`{item['full_name']}`](symbols/{slugify_symbol(item['full_name'])}.md) | "
